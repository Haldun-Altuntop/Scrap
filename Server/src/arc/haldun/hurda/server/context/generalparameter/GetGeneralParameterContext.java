...
public class GetGeneralParameterContext implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            boolean permitted = SessionManager.instance.has(requestJson.getString("session-id"));
            ...
            if (permitted) {
                ...
                String parameterName = requestJson.optString("parameter-name", "");

                GeneralParameter[] parameters;

                if (parameterName.isEmpty()) {

                    parameters = DatabaseManager.getAllGeneralParameters();

                } else {
                    parameters = new GeneralParameter[] { DatabaseManager.getGeneralParameter(parameterName)};
                }

                for (int i = 0; i < parameters.length; i++) {
                    responseJson.put(String.valueOf(i), parameters[i].toJson());
                }
            }
